...
object PlacedFlow:
  ...
  extension [Remote <: Peer, Value: Codec](using pf: PlacedFlow)(placedFlow: Flow[Value] on Remote)
    ...
    inline def locally[Local <: TiedWith[Remote]](using ps: PeerScope[Local], net: Network): Flow[(net.effect.Id, Value)] =
      pf.effect.locally[Remote, Local, Value](peer[Remote])(placedFlow)
  ...
  trait Effect extends Placement:
    ...
    /**
     * Extract the placed flow. This method can only be called by the peer that owns the flow. Any attempt to call this method on a placed flow
    ...
    /**
     * Given a placed flow on a remote peer tied with the local peer, create a flow that emits values from all instances of the remote flow.
     * Each emitted value is paired with the ID of the remote peer that produced it. Any call to this method on a placed flow that is not on
     * a remote peer or not tied with the local peer will result in a compile-time error.
     *
     * @param placedFlow
     *   the placed flow to fetch values from.
     * @return
    ...
    /**
     * Given a placed flow on a remote peer tied with the local peer, create a local flow that receives values from the remote flow.
     * This method can only be called if the local peer is tied to a single remote peer. Any call to this method on a placed flow that
     * is not on a remote peer or not tied with the local peer will result in a compile-time error.
     *
     * @param placedFlow
    ...
    /**
     * Given a placed flow on multiple remote peers tied with the local peer, create a flow that emits values from all instances of the
     * remote flows. Each emitted value is paired with the ID of the remote peer that produced it. This method can only be called if the
     * local peer is tied to multiple remote peers. Any call to this method on a placed flow that is not on a remote peer or not tied
