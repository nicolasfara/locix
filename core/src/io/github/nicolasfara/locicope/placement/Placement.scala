...
import io.github.nicolasfara.locicope.network.Network.{reachablePeers, receive}
...
object PlacedValue:
  ...
  extension [Remote <: Peer](using pl: PlacedValue)(placedValue: pl.effect.on[pl.effect.Value, Remote])
    ...
    def asLocal[Local <: TiedToSingle[Remote]](using PeerScope[Local], Network, Codec[pl.effect.Value]): pl.effect.Value =
      pl.effect.asLocal[Remote, Local](placedValue)

    def asLocalAll[Local <: TiedToMultiple[Remote]](using
        ps: PeerScope[Local],
        net: Network,
        cdc: Codec[pl.effect.Value],
    ): Map[net.effect.Id, pl.effect.Value] =
      pl.effect.asLocalAll[Remote, Local](placedValue)
  ...
  private class EffectImpl[V] extends Effect:
    ...
    override def unwrap[Local <: Peer](using PeerScope[Local])(placedValue: Value on Local): Value =
      ...
      placedValue.runtimeChecked match
        case Placed.Local(value, _) => value
    ...
    override def locally[Remote <: Peer, Local <: TiedWith[Remote]](using
      ...
      peers.map: address =>
        val id = address.id
        val receivedValue = receive(address).fold(error => throw error, identity)
        id -> receivedValue
      .toMap
    ...
    override def asLocal[Remote <: Peer, Local <: TiedToSingle[Remote]](using
        ...
        Network,
        Codec[Value],
    )(placedValue: Value on Remote): Value = ???

    override def asLocalAll[Remote <: Peer, Local <: TiedToMultiple[Remote]](using
        ps: PeerScope[Local],
        net: Network,
        cdc: Codec[Value],
    )(placedValue: Value on Remote): Map[net.effect.Id, Value] = ???
  end EffectImpl
  ...
  trait Effect extends Placement:
     ...
     */
    def asLocal[Remote <: Peer, Local <: TiedToSingle[Remote]](using PeerScope[Local], Network, Codec[Value])(placedValue: Value on Remote): Value
