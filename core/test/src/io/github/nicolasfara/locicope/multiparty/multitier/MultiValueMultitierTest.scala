...
class MultiValueMultitierTest extends AnyFlatSpecLike, Matchers, Stubs, BeforeAndAfter:
  ...
  "The Multitier capability operating on multiple values" should "return empty map when no reachable devices in the network" in:
    ...
    (net
      ...
      .returns:
        case (Reference(_, _, _), _) => Map.empty
    ...
    (net.getAllValues(_: Reference)(using _: Decoder[Int])).times shouldBe 1
  it should "return a map with values from all reachable devices in the network" in:
    (net.registerValue(_: Int, _: Reference)(using _: Encoder[Int])).returns(_ => ())
    (net
      .getAllValues(_: Reference)(using _: Decoder[Int]))
      .returns:
        case (Reference(_, _, _), _) => Map(1 -> 10, 2 -> 10, 3 -> 10)
  ...
  it should "return a flow which is the combination of the flows returned by the connected peers" in:
    ...
    (net.registerValue(_: Int, _: Reference)(using _: Encoder[Int])).returns(_ => ())
    (net
      .getAllFlows(_: Reference)(using _: Decoder[Int]))
      .returns:
        case (Reference(_, _, _), _) => Right(Flow.fromIterable(sequenceToReturn))
    ...
    (net.registerFlow(_: Flow[Int], _: Reference)(using _: Encoder[Int])).times shouldBe 0 // On server, no flow registered
    (net.registerValue(_: Int, _: Reference)(using _: Encoder[Int])).times shouldBe 1
    (net.getAllFlows(_: Reference)(using _: Decoder[Int])).times shouldBe 1
