...
object DistanceToWithObstacle:
  ...
  def distanceTo(isSource: Boolean, isObstacle: Boolean)(using Network, Collective, PlacedFlow) = collective[Node](1.second):
    branch(isObstacle) { Double.PositiveInfinity} {
  ...
  def distanceToApp(using Network, Collective, PlacedFlow, PlacedValue) =
    val distanceFlow = distanceTo(localAddress == "node-4", localAddress == "node-2")
    val unitOf = on[Node]:
