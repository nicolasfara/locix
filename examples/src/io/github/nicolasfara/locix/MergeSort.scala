 ...
 *
 * Three peers (Alice, Bob, Carol) cooperatively sort a list using merge-sort.
 * Alice holds the initial unsorted list.  The list is recursively split and
 * distributed to the three peers in a round-robin fashion:
 *
 *   Sort@Alice → prefix to Bob  (Sort@Bob),  suffix to Carol (Sort@Carol)
 *   Sort@Bob   → prefix to Carol(Sort@Carol), suffix to Alice (Sort@Alice)
 *   Sort@Carol → prefix to Alice(Sort@Alice), suffix to Bob   (Sort@Bob)
 *
 * After sorting the halves a distributed merge combines them back, always
 * producing the result at the "primary" peer for that rotation.
 *
 * Because the Scala framework uses structural peer types (not generic type
 * parameters like Rust's `Sort<A, B, C>`), each rotation is written as an
 * explicit function.  The communication topology and control-flow are
 * identical to the original Rust implementation.
 */
object MergeSort:
  ...
  /** Sort a list located at Alice, using Bob and Carol as helpers.  Result at Alice.
   *  Corresponds to `Sort<Alice, Bob, Carol>` in the Rust implementation.
   */
  ...
  /** Sort a list located at Bob, using Carol and Alice as helpers.  Result at Bob.
   *  Corresponds to `Sort<Bob, Carol, Alice>` in the Rust implementation.
   */
  ...
  /** Sort a list located at Carol, using Alice and Bob as helpers.  Result at Carol.
   *  Corresponds to `Sort<Carol, Alice, Bob>` in the Rust implementation.
   */
  ...
  /** Merge sorted prefix@Bob and suffix@Carol → result@Alice.
   *  Corresponds to `Merge<Bob, Carol>` (result at Alice) in Rust.
   */
  def mergeBC(prefixAtB: List[Long] on Bob, suffixAtC: List[Long] on Carol)(using Network, Placement, Choreography): List[Long] on Alice = Choreography:
    val isPrefixEmpty: Boolean on Bob = on[Bob]:
      take(prefixAtB).isEmpty
    val isPrefixEmptyAll = broadcast[Bob, Boolean](isPrefixEmpty)
    if isPrefixEmptyAll then
      comm[Carol, Alice](suffixAtC)
    else
      ...
      if isSuffixEmptyAll then
        comm[Bob, Alice](prefixAtB)
      else
  ...
  /** Merge sorted prefix@Carol and suffix@Alice → result@Bob.
   *  Corresponds to `Merge<Carol, Alice>` (result at Bob) in Rust.
   */
  def mergeCA(prefixAtC: List[Long] on Carol, suffixAtA: List[Long] on Alice)(using Network, Placement, Choreography): List[Long] on Bob = Choreography:
  ...
  /** Merge sorted prefix@Alice and suffix@Bob → result@Carol.
   *  Corresponds to `Merge<Alice, Bob>` (result at Carol) in Rust.
   */
  def mergeAB(prefixAtA: List[Long] on Alice, suffixAtB: List[Long] on Bob)(using Network, Placement, Choreography): List[Long] on Carol = Choreography:
